...
function abstet {
	...
	local -A opt=(
		...
		# Фон
		[background_path]="$(readlink -f $(dirname $0))/backgrounds/18+"		# Путь к каталогу с ascii изображениями: путь к текущему скрипту + ./backgrounds
	...
	# Время (в секундах) падения фигуры на дно для каждого уровня в оригинальном Тетрисе в версии NTSC:
...
function main {
	...
	local __this_filename=$(basename $0)															# Имя файла текущего скрипта
	local options																					# Строка с обработанными опциями, в итоге передаётся в функцию игры
	local fl_help																					# Флаг вывода помощи
	...
	options=$(getopt --unquoted --options 'a:c:hl:s' --longoptions 'align:,background-path:,columns:,help,lines:,show-hotkeys' --name "$__this_filename" -- "$@")
	if [[ $? -ne 0 ]]; then
		# 102 - ошибка функции getopt при обработке аргументов функции main
	...
	if [[ -n "$fl_help" || "$#" -ne 0 || -n "$fl_error" ]]; then
		...
		echo ""
		echo "Использование: ${__this_filename} [ОПЦИЯ] ..."
		echo ""
		echo "Опции:"
